<?
	...
	$r_academic				= $row[jobseeker_resume_academic];
	$r_workingyear			= $row[jobseeker_resume_workingyear];
	$r_expectedsalary		= $row[jobseeker_resume_expectedsalary];
	$r_availability			= $row[jobseeker_resume_availability];
	$r_availability_date	= $row[jobseeker_resume_availabilitydate];
	
	$r_industry				= "-". $row[jobseeker_interest_industry];
	$r_jobfunction			= "-". $row[jobseeker_interest_jobfunction];
	$r_position				= $row[jobseeker_interest_position];
	$r_employmentterm		= "-" . $row[jobseeker_interest_employmentterm];
	$r_joblocation			= "-" . $row[jobseeker_interest_joblocation];


	$sql_query  			= "SELECT TO_DAYS(NOW()) - TO_DAYS(jobseeker_birthdate) AS age_days FROM jobseeker WHERE jobseeker_id = '$clogin_jobseeker'";
	$result					= mysql_query($sql_query) or die(mysql_error());
	$row 					= mysql_fetch_array($result);
	...
	$sql_query				= "SELECT * FROM setup_academic WHERE academic_id = '$r_academic'";
	$result					= mysql_query($sql_query) or die(mysql_error());
	$row					= mysql_fetch_array($result);
	$r_academic_value		= $row[academic_order];

	

	$sql_tables	= "job , employer";
	if ($r_academic_value	) { $sql_tables .= ", setup_academic		"; }
	...
	include("system_jobseeker_matches.php");
	...
	$i			= 0;
	$sql_query .= " ORDER BY job_date_add DESC LIMIT $current_item, $row_admin";
	$result		= mysql_query($sql_query) or die(mysql_error());
	...
	$current_url		= "jobseeker_matches.php";
	$this_url			= $_SERVER['REQUEST_URI'];
	$this_url_encoded	= htmlentities(urlencode($this_url));
	$search_detail 		= "";
	...
	$smarty->display('jobseeker_matches.html');	
