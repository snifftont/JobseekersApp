<?
	...
	if ($skeyword != "keywords here ..." && $skeyword != "") { $stitle = $skeyword; }
	else { $skeyword = ""; }
	...
	if (!$sorder_by		) { $sorder_by		= "date_add";	}
	if (!$sorder_method	) { $sorder_method	= "desc";		}

	
	
	$sql_tables	= "tmp_search_result, job, employer, setup_jobfunction, setup_industry";
	if ($setup['enable_job_term'] 				== "Y"	) { $sql_tables	.= ", setup_term			"; 	}
	if ($setup['enable_job_office_country'] 	== "Y"	) { $sql_tables	.= ", setup_country			"; 	}
	if ($setup['enable_job_office_location'] 	== "Y"	) { $sql_tables	.= ", setup_joblocation		"; 	}


	
	$sql_where	= "";
	$sql_where .= " result_search 		=  '$search_id'		";
	$sql_where .= " AND result_job 		=   job_id			";
	$sql_where .= " AND job_employer 	=   employer_id		";
	$sql_where .= " AND job_function 	=   jobfunction_id	";
	$sql_where .= " AND job_industry 	=   industry_id		";

	if ($setup['enable_job_term'] 				== "Y"	) { $sql_where	.= " AND job_term 				=  term_id			"; }
	if ($setup['enable_job_office_country'] 	== "Y"	) { $sql_where	.= " AND job_office_country 	=  country_id		"; }
	if ($setup['enable_job_office_location'] 	== "Y"	) { $sql_where	.= " AND job_office_location 	=  joblocation_id	"; }
	
	
	

	$sql_order	= "";
	if ($sorder_by	== "number"		) { $sql_order = "job_id"; 				}
	if ($sorder_by	== "title"		) { $sql_order = "job_title"; 			}
	if ($sorder_by	== "term"		) { $sql_order = "term_name"; 			}
	if ($sorder_by	== "company"	) { $sql_order = "employer_company"; 	}
	if ($sorder_by	== "category"	) { $sql_order = "jobfunction_name"; 	}
	if ($sorder_by	== "salary"		) { $sql_order = "job_salary_min"; 		}
	if ($sorder_by	== "date_add"	) { $sql_order = "job_date_add"; 		}
	if ($sorder_by	== "date_end"	) { $sql_order = "job_lastdate"; 		}
	if ($sorder_by	== "distance"	) { $sql_order = "result_distance"; 	}
	$sql_order	= "$sql_order $sorder_method";
    ...
    $sql_query	= "SELECT * FROM $sql_tables WHERE $sql_where";
	...
	$i			= 0;
    $sql_query	= "SELECT * FROM $sql_tables WHERE $sql_where ORDER BY $sql_order LIMIT $current_item, $front_search_rows";
	$result		= mysql_query($sql_query) or die(mysql_error());
	while($row	= mysql_fetch_array($result))	{
		...
		$job_office_distance		= $row[result_distance];
		$job_office_distance		= number_format($job_office_distance, 2, $web_decimal_separator, $web_thousand_separator);
		...
		$arr_job_office_distance[$i]	= $job_office_distance;
	...
	$current_url			= "site_search.php";
	$this_url				= $_SERVER['REQUEST_URI'];
	$this_url_encoded		= htmlentities(urlencode($this_url));
	$search_advanced_url	= "site_search_advanced.php?" . $search_detail;
	

	if (!$current_page						)	{ $current_page = 1; 									} 
	if (!$page_url							)	{ $page_url 	= $current_url; 						} 
	if ($job_total % $front_search_rows > 0	)	{ $total_page 	= floor($job_total / $row_admin) + 1;	} 
	...
	$smarty->assign("job_office_distance"			, $arr_job_office_distance		);
	...
	$smarty->assign("search_advanced_url"			, $search_advanced_url			);
	$smarty->assign("show_distance"					, $show_distance				);
